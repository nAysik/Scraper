...
export default function TwitchDiscoveryPanel() {
  ...
  const [mode, setMode]               = useState<'live' | 'vods'>('live');
  ...
  const columns = useMemo<ColumnDef<TwitchRow>[]>(() => [
    ...
    },
    {
      ...
      header: mode === 'vods' ? 'VOD views' : 'Live viewers',
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-1 w-fit rounded-md border border-gray-700 p-0.5 bg-gray-900">
        ...
        <button
          type="button"
          onClick={() => { setMode('vods'); setRows([]); setSearched(false); }}
          className={`px-3 py-1 text-sm rounded transition-colors ${mode === 'vods' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-gray-200'}`}
        >
          Recent VODs
        </button>
      </div>
      ...
      {searched && (
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <label className="text-xs text-gray-400 whitespace-nowrap">{mode === 'vods' ? 'Max VOD views' : 'Max live viewers'}</label>
            <Input
              type="number"
