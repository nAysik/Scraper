# Email Campaigns — Plan 3: Campaigns UI

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the CampaignsPanel component and wire it into OutreachTabs as the 5th tab.

**Architecture:** `src/components/outreach/campaigns-panel.tsx` is a single client component managing three views (list / compose / detail) via local state. It fetches from the Plan 2 API routes. `OutreachTabs` in `discovery-table.tsx` gets a 5th tab `'campaigns'` and renders `<CampaignsPanel />`.

**Tech Stack:** React, TanStack Table, shadcn/ui (Button, Input, Badge, Table, Textarea), lucide-react.

**Depends on:** Plan 2 (all API routes exist).
...
| File | Action |
|------|--------|
| `src/components/outreach/campaigns-panel.tsx` | Create |
| `src/components/outreach/discovery-table.tsx` | Modify — add 5th tab |

---

## Task 1: `CampaignsPanel` component

**Files:**
- Create: `src/components/outreach/campaigns-panel.tsx`
...
export default function CampaignsPanel() {
  ...
  // ── Preview (first selected channel) ──
  ...
  function toggleAll() {
    if (selectedIds.size === channels.length) {
      ...
      setSelectedIds(new Set(channels.map(c => c.youtubeId)));
    }
  }
    ...
    try {
      const createRes = await fetch('/api/campaigns', {
        ...
        body: JSON.stringify({
          ...
          bodyHtmlTemplate: bodyHtml || `<pre style="font-family:sans-serif">${bodyText}</pre>`,
      ...
      setSendResult({ sent: sendData.sent, failed: sendData.failed });
  ...
  if (view === 'list') {
    return (
      <div className="space-y-4">
        ...
        {listLoading ? (
          <div className="py-12 flex justify-center"><Loader2 className="animate-spin h-5 w-5 text-gray-400" /></div>
        ) : campaigns.length === 0 ? (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        {channelsLoading ? (
          <div className="py-4 flex justify-center"><Loader2 className="animate-spin h-4 w-4 text-gray-400" /></div>
        ) : (
          <div className="rounded-md border border-gray-800 overflow-hidden max-h-56 overflow-y-auto">
            <Table>
              <TableHeader>
                <TableRow className="border-gray-800 hover:bg-transparent sticky top-0 bg-black">
              ...
              <TableBody>
                {channels.map(c => (
                  <TableRow key={c.youtubeId} className="border-gray-800 hover:bg-gray-900 cursor-pointer" onClick={() => toggleChannel(c.youtubeId)}>
                    <TableCell className="py-1.5">
                      <input type="checkbox" checked={selectedIds.has(c.youtubeId)} onChange={() => toggleChannel(c.youtubeId)} onClick={e => e.stopPropagation()} />
                    </TableCell>
      ...
      <div className="space-y-2">
        <label className="text-xs text-gray-400 uppercase tracking-wide">
          Template — use {'{{ChannelName}}'}, {'{{TopGames}}'}, {'{{Genre}}'}, {'{{Platform}}'}
        </label>
        <Input
          value={subject}
          onChange={e => setSubject(e.target.value)}
          placeholder="Subject: e.g. Collaboration opportunity for {{ChannelName}}"
          className="bg-gray-900 border-gray-700 text-white placeholder:text-gray-500"
        />
        <Textarea
          value={bodyText}
          onChange={e => setBodyText(e.target.value)}
          placeholder={"Hi {{ChannelName}},\n\nI came across your channel covering {{TopGames}} and think there could be a great fit...\n\nBest,\n{{Platform}} outreach"}
          rows={8}
      ...
      {previewChannel && (subject || bodyText) && (
        <div className="rounded-md border border-gray-700 bg-gray-900 p-4 space-y-2">
          <p className="text-xs text-gray-500 uppercase tracking-wide">Preview — {previewChannel.name}</p>
          {subject && <p className="text-white text-sm font-medium">{substitutePreview(subject, previewChannel)}</p>}
          {bodyText && <pre className="text-gray-300 text-sm whitespace-pre-wrap font-sans">{substitutePreview(bodyText, previewChannel)}</pre>}
        </div>
      )}
      ...
      {sendResult ? (
        <div className="rounded-md border border-gray-800 bg-gray-900 p-4 text-sm space-y-1">
          <p className="font-medium text-white">Send complete — {sendResult.sent} sent{sendResult.failed > 0 ? `, ${sendResult.failed} failed` : ''}</p>
        </div>
      ) : (
        <div className="flex items-center gap-3">
          <Button
          ...
          >
            ...
            {creating ? 'Creating…' : sending ? `Sending… (~${estimatedMinutes} min)` : (
              <><Send className="h-4 w-4 mr-1" />Create & send ({selectedIds.size})</>
            )}
          </Button>
          {sending && (
            <span className="text-xs text-gray-500">
              Sending at 12 s/email to stay within one.com rate limit
            </span>
          )}
        </div>
      )}
...
Expected: no output. If `Textarea` is missing from `@/components/ui/textarea`, install it:
```powershell
$env:PATH = "C:\Program Files\nodejs;" + $env:PATH
npx.cmd shadcn@latest add textarea
```

- [ ] **Step 3: Commit**

```bash
git add src/components/outreach/campaigns-panel.tsx
git commit -m "feat(ui): CampaignsPanel — campaign list, compose, send, detail views"
```

---

## Task 2: Wire into OutreachTabs as 5th tab

**Files:**
- Modify: `src/components/outreach/discovery-table.tsx`

Read the file first. The `OutreachTabs` function currently has tab union `'discover' | 'enrich' | 'outreach-list' | 'twitch-discover'` and four tab buttons.

- [ ] **Step 1: Add import**

At the top of `discovery-table.tsx`, alongside the other component imports, add:
...
- [ ] **Step 3: Add 5th tab button**

After the "Discover on Twitch" button, add:
...
After the `{tab === 'twitch-discover' && <TwitchDiscoveryPanel />}` line, add:
...
```bash
git add src/components/outreach/discovery-table.tsx
git commit -m "feat(ui): add Campaigns tab to OutreachTabs"
```
